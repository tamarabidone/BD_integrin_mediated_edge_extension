...
clear; 
clc;
SaveDirectory = '/Users/keithcarney/Dropbox/Desktop/University of Utah Work Stuff/Bidone Lab';    
    
   % Setup all combinations of parameters to be varied  -------------------------------------------------------------------
    values = [];
    nRuns = 10; % Total number of runs for each condition
    k_a = [1000,100,10, 1, 0.1, 0.01, 0.001]; % adhesion spring constant
    peak = [1,2];   % WT or Mn
    nLigands = 100:200:1000;
    ...
    %for k = 1:size(values,1)   
           ...
           % Special Conditions (or edits to ModelParameters) ==============================================================
            ...
            SaveName = ['SIMULATION-001__','Ka_',SimFormat(values(k,1)),'__Peak_',sprintf('%02d',values(k,2)),'__nLigands_',sprintf('%04d',values(k,3)),'__test_run_',sprintf('%02d',values(k,4)),'.mat'];
